...
with open('data/w2e2.pkl', 'rb') as fo:
    w2e = pickle.load(fo)
...
with open('data/e2w2.pkl', 'rb') as fo:
    e2w = pickle.load(fo)
...
def createTranslationMap(inputDir):
    ...
    w2e = {}
    e2w = {START_SYMBOL: '^', END_SYMBOL: '$', PAD_SYMBOL: '#'}
    for i, w in enumerate(s):
        w2e[w] = i
        e2w[i] = w

    # print("reserve -1 for START, -2 for END, -3 for PAD")
...
def saveTranslationMap():
    ...
    with open('data/w2e2.pkl', 'wb') as fo:
        pickle.dump(w2e, fo)
    with open('data/e2w2.pkl', 'wb') as fo:
        pickle.dump(e2w, fo)
...
def tfrecordFromTrain():
    inputDir = 'data'
    outputFile = 'data/train2.tfrecord'
    ...
    for rec in capj:
        ...
        for cap in rec['caption']:
            c = encodeCaption(cap) + [END_SYMBOL]
            len_c = len(c) + 1
            c = c + [PAD_SYMBOL] * (MAX_CAPTION_LEN - len(c))
